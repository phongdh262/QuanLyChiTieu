                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                    ...
                    <div className="px-3 pt-5 pb-2">
                        <button
                            ...
                            className="w-full flex items-center justify-between mb-2 px-2 group"
                        >
                            <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/30">{t('sheets')}</h3>
                        ...
                        {/* Active Sheet — always visible */}
                        {activeSheet && (
                            <button
                                key={activeSheet.id}
                                onClick={() => { if (sheets.length > 1) setSheetsExpanded(prev => !prev); }}
                                className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-left transition-all duration-150 bg-indigo-500/20 text-indigo-300 border border-indigo-400/20"
                            >
                                ...
                                {sheets.length > 1 && (
                                    <span className="ml-auto text-[9px] font-bold text-white/20 bg-white/[0.04] px-1.5 py-0.5 rounded">
                                        {sheets.length}
                                    </span>
                                )}
                            </button>
                        ...
                        {/* Other Sheets — expandable */}
                        {sheetsExpanded && otherSheets.length > 0 && (
                            <div className="space-y-0.5 mt-1 animate-in slide-in-from-top-1 fade-in duration-200">
                                {otherSheets.map(sheet => (
                                    <button
                                        key={sheet.id}
                                        onClick={() => { onSheetChange(sheet.id); setSheetsExpanded(false); if (window.innerWidth < 1024) onToggle(); }}
                                        className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-left transition-all duration-150 text-white/60 hover:bg-white/[0.06] hover:text-white/80"
                                    >
