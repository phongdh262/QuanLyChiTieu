...
export default function ActivityLogList({ members, month, year, sheetId, sheetName }: Props) {
    ...
    return (
        <Card className="w-full overflow-hidden border-none shadow-none bg-transparent flex flex-col h-full">
            <CardHeader
                className="p-4 cursor-pointer hover:bg-indigo-50/30 dark:hover:bg-indigo-500/5 transition-all duration-500 flex flex-row items-center justify-between gap-2 border-b border-indigo-50/50 dark:border-white/[0.06] shrink-0"
            ...
            >
                ...
                <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                    <Select value={selectedUser} onValueChange={setSelectedUser}>
                        <SelectTrigger className="w-[140px] h-9 bg-slate-50 dark:bg-white/[0.06] border-slate-200 dark:border-white/[0.08] rounded-lg font-semibold text-slate-700 dark:text-slate-300 focus:ring-purple-100 shadow-sm hover:bg-slate-100 dark:hover:bg-white/[0.1] text-xs">
                    ...
                    <div className="h-full overflow-y-auto p-4 space-y-3 custom-scrollbar">
                        {isLoading && logs.length === 0 ? (
                            ...
                            filteredLogs.map(log => (
                                <div
                                    key={log.id}
                                    className="p-3 rounded-lg border border-slate-100 dark:border-white/[0.06] bg-white dark:bg-white/[0.03] shadow-sm hover:shadow-md transition-shadow duration-200 flex flex-col gap-1.5"
                                >
