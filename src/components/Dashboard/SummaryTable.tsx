...
export default function SummaryTable({ members, calculations }: Props) {
    ...
    return (
        <Card className="premium-card overflow-hidden border-none soft-shadow mb-6 group/summary">
            <CardHeader className="cursor-pointer flex flex-row items-center justify-between pb-5 bg-gradient-to-br from-white dark:from-slate-500/5 via-sky-50/30 dark:via-transparent to-blue-50/25 dark:to-transparent border-b border-slate-200/70 dark:border-white/[0.06]" onClick={() => setIsOpen(!isOpen)}>
                ...
                <div className={cn("rounded-full p-2 bg-white dark:bg-white/[0.06] shadow-sm text-slate-400 ring-1 ring-slate-200 dark:ring-white/[0.06] transition-transform duration-300 hover:bg-blue-50 dark:hover:bg-blue-500/10 hover:text-blue-600", isOpen && "rotate-180 bg-blue-100 dark:bg-blue-500/20 text-blue-600")}>
            ...
            {isOpen && (
                <CardContent className="fade-in pt-0 pb-2">
                    <div className="overflow-x-auto">
                        <Table>
                            <TableHeader className="bg-slate-50/40 dark:bg-white/[0.02]">
                                <TableRow>
